...
public class LabelStar extends Label{
	...
	public LabelStar(Node node,boolean marque,double cout,Arc last_arc, boolean shortestMode, Node destination ) {
		super(node,marque,cout,last_arc);
		double cost = node.getPoint().distanceTo(destination.getPoint());
		if (shortestMode) {
			this.estimatedCost = cost;
		}else {
			// Fastest Mode
			// 100 m.s-1 choice of speed
